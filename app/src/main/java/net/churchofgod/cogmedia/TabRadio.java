...
import net.churchofgod.cogmedia.adapter.onlineAdapter;
...
public class TabRadio extends Fragment {
    ...
    GridView gridView;
    onlineAdapter myGridAdapter;
    private static String DATA_URL = "http://app.trumpetradio.com/musicApp/backend/web/index.php/api/book_by_category_title?categoryId=47&page=*&platform=1";
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if(allImageArray != null && allImageArray.length() > 0){
                        ...
                        ArrayList<onlineModel> imageObjects = new ArrayList<>();
                        for(int i = 0; i < allImageArray.length();i++){
                            JSONObject jsonItem = allImageArray.optJSONObject(i);

                            imageObjects.add(new onlineModel(jsonItem));
                        }

                        myGridAdapter= new onlineAdapter(getActivity().getApplicationContext(), imageObjects);
